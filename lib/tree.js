...
class Tree {
  ...
  static async verify(proof, key, root) {
    try {
      const [exists, value] = await nurkel.verify(proof, key, root);
      return [URKEL_OK, value, exists];
    } catch (e) {
      return [codes[e.code], null, false];
    }
  }
  ...
  static verifySync(proof, key, root) {
    try {
      const [exists, value] = nurkel.verify_sync(proof, key, root);
      return [URKEL_OK, value, exists];
    } catch (e) {
      return [codes[e.code], null, false];
    }
