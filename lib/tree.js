...
class Tree {
  ...
  debugInfoSync(showTransactions = false) {
    ...
    const obj = nurkel.tree_debug_info_sync(this.tree, showTransactions);
    for (const tx of obj.transactions)
      tx.state = states[tx.state];

    return {
      ...obj,
      state: states[obj.state]
    };
  }
...
class Snapshot {
  ...
  /**
   * @param {Number} [cacheSize = 1] - Cache size for bulk reads.
   */
...
class Iterator {
   ...
   * @param {Number} [cacheSize=1] - Number of elements to read at once.
   */

  constructor(tx, cacheSize = 1) {
    assert(tx.tx != null);
